...
const generateMoodData = (userId, days) => {
  const moodLogs = [];
  ...
  for (let i = 0; i < days; i++) {
    ...
    const moods = moodData.moods[score];
    const emojis = moodData.emojis[score];
...
const generateSleepData = (userId, days) => {
  const sleepLogs = [];
...
const generateActivityData = (userId, days) => {
  const activities = [];
  ...
  for (let i = 0; i < days; i++) {
    ...
    for (let j = 0; j < numActivities; j++) {
      ...
      const categoryActivities = activityData.categories[category];
