...
export default function AddEntryScreen() {
  ...
  const [overallMoodCardExpanded, setOverallMoodCardExpanded] = useState<boolean>(false);
  const [overallMoodScore, setOverallMoodScore] = useState<number>(3);
  ...
  const handleOverallMoodRating = (score: number) => {
    setOverallMoodScore(score);
    ...
    setSelectedMoodEmoji(emojiMap[score] || '😐');
    setOverallMoodCardExpanded(false);
    setMoodNotesCardExpanded(true);
  };

  // Auto-expand overall mood card when emotion triggers are set up
  useEffect(() => {
    if (emotionTriggers.length > 0 && !overallMoodCardExpanded && overallMoodScore === 3) {
      // Only auto-expand if user hasn't set a rating yet
      setOverallMoodCardExpanded(true);
    }
  }, [emotionTriggers.length]);
  ...
  const handleSaveEntry = async () => {
    ...
    try {
      ...
      const finalMoodScore = overallMoodScore > 0 ? overallMoodScore : Math.round(selectedMoods.reduce((sum, mood) => sum + mood.id, 0) / selectedMoods.length);
      const finalMoodEmoji = selectedMoodEmoji !== '😐' || overallMoodScore > 0 ? selectedMoodEmoji : (selectedMoods[0]?.emoji || '😐');
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* CARD 3: Rate Overall Mood */}
        {emotionTriggers.length > 0 && (
          <View style={styles.card}>
            <TouchableOpacity
              style={styles.cardHeader}
              onPress={() => setOverallMoodCardExpanded(!overallMoodCardExpanded)}
            >
              <View style={styles.cardTitleContainer}>
                <Heart size={20} color="#F59E0B" />
                <Text style={styles.cardTitle}>How would you rate your overall mood?</Text>
                {overallMoodScore > 0 && overallMoodScore !== 3 && (
                  <Text style={{ marginLeft: 8, fontSize: 20 }}>{selectedMoodEmoji}</Text>
                )}
              </View>
              {overallMoodCardExpanded ? (
                <ChevronUp size={20} color="#6B7280" />
              ) : (
                <ChevronDown size={20} color="#6B7280" />
              )}
            ...
            {overallMoodCardExpanded && (
              <View style={styles.cardContent}>
                <View style={styles.overallMoodRatingContainer}>
                  {[
                    ...
                    <TouchableOpacity
                      key={score}
                      style={[
                        styles.overallMoodButton,
                        overallMoodScore === score && styles.selectedOverallMoodButton,
                      ]}
                      onPress={() => handleOverallMoodRating(score)}
                    >
                      <Text style={styles.overallMoodEmoji}>{emoji}</Text>
                      <Text
                        style={[
                          styles.overallMoodLabel,
                          overallMoodScore === score && styles.selectedOverallMoodLabel,
                        ]}
                      >
                        {label}
                      </Text>
                            ...
                            <View style={styles.activityDetailsContainer}>
                              ...
                              <View style={styles.durationContainer}>
                                ...
                                <TextInput
                                  ...
                                  value={item.duration || ''}
                                  onChangeText={text =>
                                    handleActivityDuration(category.id, item.id, text)
                                  }
          ...
          {productivityCardExpanded && (
            <View style={styles.cardContent}>
              ...
              <View style={styles.focusedHoursContainer}>
                <Text style={styles.focusedHoursLabel}>How many hours of focused work?</Text>
              ...
              <Text style={styles.factorsQuestion}>
                What factors influenced your productivity?
              </Text>
